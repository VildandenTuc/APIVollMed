...
public class PacienteController {
    ...
    public ResponseEntity registrarPaciente(@RequestBody @Valid DatosRegistroPaciente datos, UriComponentsBuilder uriComponentsBuilder){
        ...
        var uri = uriComponentsBuilder.path("/pacientes/{id}").buildAndExpand(paciente.getId());
        return ResponseEntity.created(uri.toUri()).body(new DatosDetalladoPaciente(paciente));
    }

    @GetMapping
    public ResponseEntity<Page<DatosListadoPaciente>> listarPaciente(@PageableDefault(page = 0, size = 10, sort = {"nombre"}) Pageable paginacion) {
